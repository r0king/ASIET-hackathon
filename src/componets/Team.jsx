...
const Team = () => {
    ...
    <svg width={200} height={600} fill="none" viewBox="0 0 200 600" aria-hidden="true">
      <defs>
        <pattern id="8b1b5f72-e944-4457-af67-0c6d15a99f38" x={0} y={0} width={40} height={40} patternUnits="userSpaceOnUse">
          <rect x={0} y={0} width={5} height={5} className="text-red-500" fill="currentColor" />
        </pattern>
      </defs>
  ...
  return (
    <div className="relative mx-auto py-10 px-1 sm:px-4 w-full max-w-7xl bg-transparent">
      <div className="flex flex-col items-center">

        {/* :TITLE CONTAINER */}
        <h2 className="mb-6 text-lg text-white font-semibold uppercase tracking-widest">
          <span className="text-red-500">Who </span> are we?
        </h2>
            ...
            {people.map((person, index) => (
              <li key={index} className="col-span-5 md:col-span-3 lg:col-span-2 py-8 px-2">
                ...
                <div className={`group z-10 relative ${index % 2 === 0 ? "bottom-3" : "top-3"} w-full h-full inline-flex flex-col justify-center transition-all duration-300 ease-in transform hover:translate-y-3`}>
                  {/* :::top selector line */}
                  <span className="z-20 absolute -top-16 left-1/2 w-0.5 h-10 bg-red-500 bg-opacity-40 opacity-0 transition-all duration-150 ease-out transform -translate-x-1/2 group-hover:translate-y-10 group-hover:opacity-100" />
                  {/* :::picture */}
                  ...
                  {/* :::name */}
                  <h3 className="text-sm text-red-500 font-bold uppercase tracking-wider transition-all duration-300 transform -translate-y-20 opacity-0 group-hover:-translate-y-0 group-hover:opacity-100">{person.name}</h3>
                  {/* :::role */}
